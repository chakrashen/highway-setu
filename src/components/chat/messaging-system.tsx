...
export function MessagingSystem() {
  ...
  return (
    <div className="flex h-[600px] glass-strong border border-foreground/10 rounded-2xl overflow-hidden">
      {/* Sidebar */}
      <div className="w-1/3 min-w-[280px] border-r border-foreground/10 flex flex-col bg-background/30">
        <div className="p-4 border-b border-foreground/10">
          ...
          <div className="relative">
            ...
            <input 
              ...
              className="w-full bg-foreground/5 border border-foreground/10 rounded-xl py-2 pl-9 pr-4 text-sm text-foreground focus:outline-none focus:border-blue/50"
            />
          </div>
        </div>
        <div className="flex-1 overflow-y-auto hidden-scrollbar">
          {contacts.map((contact) => (
            ...
            >
              <div className="relative">
                <div className="w-12 h-12 rounded-full bg-gradient-to-br from-white/10 to-white/5 flex items-center justify-center font-bold text-white border border-foreground/10">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex justify-between items-baseline mb-1">
                  ...
                  <span className="text-[10px] text-foreground/40 whitespace-nowrap">{contact.time}</span>
                </div>
                <div className="flex justify-between items-center">
                  <p className={`text-xs truncate pr-2 ${contact.unread > 0 ? 'text-foreground font-medium' : 'text-foreground/50'}`}>
                    {contact.lastMessage}
                  </p>
                  {contact.unread > 0 && (
          ...
          <>
            ...
            <div className="p-4 border-b border-foreground/10 flex justify-between items-center bg-foreground/5">
              ...
              <div className="flex items-center gap-2">
                <button className="p-2 hover:bg-foreground/10 rounded-full text-foreground/60 hover:text-foreground transition-colors"><Phone className="w-4 h-4" /></button>
                <button className="p-2 hover:bg-foreground/10 rounded-full text-foreground/60 hover:text-foreground transition-colors"><Video className="w-4 h-4" /></button>
                <button className="p-2 hover:bg-foreground/10 rounded-full text-foreground/60 hover:text-foreground transition-colors"><MoreVertical className="w-4 h-4" /></button>
              </div>
            </div>

            {/* Messages */}
            <div className="flex-1 overflow-y-auto p-4 space-y-4 hidden-scrollbar">
              <div className="text-center text-[10px] text-foreground/30 my-4">Today</div>
              ...
              <div className="flex justify-end">
                <div className="max-w-[70%]">
                  ...
                  <div className="text-[10px] text-foreground/40 text-right mt-1">10:40 AM • Read</div>
                </div>
              </div>

              <div className="flex justify-start">
                <div className="max-w-[70%]">
                  <div className="bg-foreground/10 text-foreground p-3 rounded-2xl rounded-tl-sm text-sm border border-foreground/5">
                    Yes, we have parking space. I have reserved a spot for you near the security cabin.
                  </div>
                  <div className="text-[10px] text-foreground/40 mt-1">10:42 AM</div>
                </div>
              </div>
            </div>
            ...
            <div className="p-4 border-t border-foreground/10 bg-foreground/5">
              <div className="flex items-end gap-2">
                <div className="flex gap-1 pb-2">
                  <button className="p-2 text-foreground/40 hover:text-foreground transition-colors" title="Attach"><Paperclip className="w-4 h-4" /></button>
                  <button className="p-2 text-foreground/40 hover:text-foreground transition-colors" title="Location"><MapPin className="w-4 h-4" /></button>
                </div>
                <div className="flex-1 relative">
                  <textarea 
                    ...
                    className="w-full bg-background border border-foreground/10 rounded-xl py-3 pl-4 pr-12 text-sm text-foreground focus:outline-none focus:border-blue/50 resize-none max-h-32"
                    rows={1}
                  />
                  <button className="absolute right-2 bottom-3 p-1.5 text-foreground/40 hover:text-foreground transition-colors">
